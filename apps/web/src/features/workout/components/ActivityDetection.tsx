...
export const ActivityDetection = ({ activities, workout }: Props) => {
  ...
  const workoutState = useRef(useWorkout.getState());
  ...
  useEffect(
    () => useWorkout.subscribe((state) => (workoutState.current = state)),
    []
  );
  const handleDetect = useCallback((args: CheckPoseArgs) => {
    const { currentActivity, startedAt, tick } = workoutState.current;
